...
export default function MeterForm({
  ...
  return (
    <div className="card" style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {/* Meter Awal */}
      <div>
        <label style={{ fontSize: 13, fontWeight: 700, color: "var(--color-txt3)", display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
          METER AWAL (m³)
          {meterAwalAuto && (
            <span style={{ fontSize: 13, background: "rgba(21,128,61,0.12)", color: "var(--color-lunas)", padding: "2px 6px", borderRadius: 10 }}>
              Otomatis
            </span>
          )}
        </label>
      ...
      {/* Meter Sekarang */}
      <div>
        <label style={{ fontSize: 13, fontWeight: 700, color: "var(--color-txt3)", display: "block", marginBottom: 6 }}>
          METER SEKARANG (m³)
        </label>
        <input
          ref={meterAkhirRef}
        ...
        {typeof meterAkhir === "number" && typeof meterAwal === "number" && meterAkhir < meterAwal && (
          <p style={{ fontSize: 13, color: "var(--color-belum)", marginTop: 4, display: "flex", alignItems: "center", gap: 4 }}>
