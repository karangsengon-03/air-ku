    ...
    >
      ...
      >
        ...
        <div className="row-10">
          <button className="btn-secondary" style={{ flex: 1, height: 44 }} onClick={onClose}>Batal</button>
          <button className="btn-primary" style={{ flex: 2, height: 44 }} onClick={handleSave} disabled={!val.trim()}>Simpan</button>
        </div>
      </div>
    </div>
  );
...
export default function DusunRTSection({ settings, addToast, showConfirm }: DusunRTSectionProps) {
  ...
  const save = async (data: Partial<typeof settings>) => {
    setSaving(true);
    try { await updateSettings(data); addToast("success", "Tersimpan"); }
    ...
    <>
      ...
      <SettingsSection icon={<MapPin size={18} />} title="Dusun & RT">
        <div style={{ paddingTop: 12 }}>
          <button className="btn-primary" style={{ width: "100%", marginBottom: 14, height: 44, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
            ...
            <div key={dusun} style={{ marginBottom: 8, border: "1px solid var(--color-border)", borderRadius: 10, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", padding: "12px 14px", background: "var(--color-bg)", gap: 6 }}>
                ...
                >
                  ...
                  <span>{dusun}</span>
                  <span style={{ fontSize: 12, color: "var(--color-txt3)", fontWeight: 400 }}>({(rtPerDusun[dusun] || []).length} RT)</span>
                  {expandedDusun === dusun
              ...
              {expandedDusun === dusun && (
                <div style={{ padding: "14px 14px", background: "var(--color-card)", borderTop: "1px solid var(--color-border)" }}>
                  {(rtPerDusun[dusun] || []).length === 0 ? (
                    <p style={{ fontSize: 12, color: "var(--color-txt3)", marginBottom: 12, textAlign: "center" }}>Belum ada RT di dusun ini.</p>
                  ) : (
                  ...
                  <button className="btn-secondary" style={{ width: "100%", height: 40, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13 }}
